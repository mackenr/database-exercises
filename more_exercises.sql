...
-- WHERE
DESCRIBE film;
SELECT title,description,rating,length FROM film WHERE length > 3;
DESCRIBE payment;
...
DESCRIBE category;
SELECT * FROM category WHERE category_id > 4 OR name LIKE '[CST]%';
DESCRIBE staff;
CREATE TEMPORARY TABLE TempStaff AS SELECT * FROM staff;
ALTER TABLE TempStaff DROP COLUMN password;
